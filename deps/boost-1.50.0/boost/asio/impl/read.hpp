...
namespace detail
{
  ...
  template <typename AsyncReadStream, typename Elem,
      typename CompletionCondition, typename ReadHandler>
  class read_op<AsyncReadStream, riakboost::array<Elem, 2>,
      CompletionCondition, ReadHandler>
    : detail::base_from_completion_cond<CompletionCondition>
  {
  public:
    read_op(AsyncReadStream& stream, const riakboost::array<Elem, 2>& buffers,
        CompletionCondition completion_condition, ReadHandler& handler)
      : detail::base_from_completion_cond<
          CompletionCondition>(completion_condition),
        stream_(stream),
    ...
    void operator()(const riakboost::system::error_code& ec,
        std::size_t bytes_transferred, int start = 0)
    {
      typename riakboost::asio::detail::dependent_type<Elem,
          riakboost::array<riakboost::asio::mutable_buffer, 2> >::type bufs = {{
        riakboost::asio::mutable_buffer(buffers_[0]),
        riakboost::asio::mutable_buffer(buffers_[1]) }};
      std::size_t buffer_size0 = riakboost::asio::buffer_size(bufs[0]);
      std::size_t buffer_size1 = riakboost::asio::buffer_size(bufs[1]);
      std::size_t n = 0;
      switch (start)
      {
        case 1:
        ...
        for (;;)
        {
          bufs[0] = riakboost::asio::buffer(bufs[0] + total_transferred_, n);
          bufs[1] = riakboost::asio::buffer(
              bufs[1] + (total_transferred_ < buffer_size0
                ? 0 : total_transferred_ - buffer_size0),
              n - riakboost::asio::buffer_size(bufs[0]));
  ...
  //private:
    AsyncReadStream& stream_;
    riakboost::array<Elem, 2> buffers_;
    std::size_t total_transferred_;
    ReadHandler handler_;
  };
  ...
  public:
    ...
    {
      typename riakboost::asio::detail::dependent_type<Elem,
          std::array<riakboost::asio::mutable_buffer, 2> >::type bufs = {{
        riakboost::asio::mutable_buffer(buffers_[0]),
        riakboost::asio::mutable_buffer(buffers_[1]) }};
      std::size_t buffer_size0 = riakboost::asio::buffer_size(bufs[0]);
      std::size_t buffer_size1 = riakboost::asio::buffer_size(bufs[1]);
