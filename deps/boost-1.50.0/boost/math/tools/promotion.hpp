...
#include <boost/type_traits/is_floating_point.hpp> // for riakboost::is_floating_point;
#include <boost/type_traits/is_integral.hpp> // for riakboost::is_integral
#include <boost/type_traits/is_convertible.hpp> // for riakboost::is_convertible
#include <boost/type_traits/is_same.hpp>// for riakboost::is_same
#include <boost/type_traits/remove_cv.hpp>// for riakboost::remove_cv
// Boost Template meta programming:
#include <boost/mpl/if.hpp> // for riakboost::mpl::if_c.
#include <boost/mpl/and.hpp> // for riakboost::mpl::if_c.
#include <boost/mpl/or.hpp> // for riakboost::mpl::if_c.
#include <boost/mpl/not.hpp> // for riakboost::mpl::if_c.
    ...
    namespace tools
    {
      ...
      { // Promote, if necessary, & pick the wider of the two floating-point types.
        ...
        typedef typename mpl::if_<
          ...
          typename mpl::if_< typename mpl::and_<mpl::not_<is_floating_point<T2P> >, ::riakboost::is_convertible<T1P, T2P> >, T2P, T1P>::type>::type type;
      }; // promote_arg2
      // These full specialisations reduce mpl::if_ usage and speed up
      // compilation:
      ...
      struct promote_args
      {
         ...
         //
         BOOST_STATIC_ASSERT((0 == ::riakboost::is_same<type, long double>::value));
#endif
      };
